...
struct Module
{
    ...
    AddressInformation resolveAddress(uintptr_t address) const
    {
        ...
        }(scopes.empty() ? *subprogram->die() : scopes.back());
        ...
        std::for_each(scopes.begin(), scopes.end(), handleDie);
        return info;
    }
...
private:
    Module* reportModule(const ModuleFragment& module)
    {
        ...
        if (!dwflModule) {
            fprintf(stderr, "reporting %s %lx\n", module.fileName.c_str(), module.addressStart);
